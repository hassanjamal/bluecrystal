    ...
    public function postCreate()
    {
        if (Sentry::getUser()->isSuperUser()) {
            ...
            if ($this->rdscheme->id) {
                ...
                return Redirect::to('admin/rd-schemes/' . $this->rdscheme->id . '/edit')->with(
                               'success', Lang::get('admin/scheme/messages.create.success')
                );
            }
            else {
    ...
    public function getEdit($rdscheme)
    {
        if (Sentry::getUser()->isSuperUser()) {
            if ($rdscheme->id) {
                ...
                return Redirect::to('admin/rdschemes')->with(
    ...
    public function postEdit($rdscheme)
    {
        if (Sentry::getUser()->isSuperUser()) {
            ...
            if ($rdscheme->save()) {
                ...
                // Redirect to the new rdscheme page
                return Redirect::to('admin/rd-schemes/' . $rdscheme->id . '/edit')->with(
                               'success', Lang::get('admin/scheme/messages.edit.success')
                );
            }
            else {
                // Get validation errors (see Ardent package)
    ...
    public function getData()
    {
        ...
        if (Sentry::getUser()->isSuperUser()) {
            return Datatables::of($rdschemes)
                             ...
                             ->add_column(
                             'actions',
                             '<a href="{{{ URL::to(\'admin/rd-schemes/\'. $id . \'/detail\') }}}" class="iframe btn btn-xs btn-info">{{{ Lang::get(\'button.details\') }}}</a> <a href="{{{ URL::to(\'admin/rd-schemes/\'. $id . \'/edit\') }}}" class="iframe btn btn-xs btn-danger">{{{ Lang::get(\'button.edit\') }}}</a> '
                )
                             ->remove_column('id')
                             ->make();
        }
            ...
            return Datatables::of($rdschemes)
                             ...
                             ->add_column(
                             'actions',
                             '<a href="{{{ URL::to(\'admin/rd-schemes/\'. $id . \'/detail\') }}}" class="iframe btn btn-xs btn-info">{{{ Lang::get(\'button.details\') }}}</a>'
                )
                             ->remove_column('id')
                             ->make();
        }
